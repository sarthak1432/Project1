...
const generatePDF = async (data) => {
  ...
  const cOrange = [215, 115, 40];   // Muted Orange for headings
  ...
  // Using rect + triangle for the custom slanted shape
  ...
  // Summary Box (Right) - More structured
  ...
  // Left border for summary box to make it more professional
  ...
  doc.setFontSize(9);
  doc.setTextColor(...cTextBlack);
  doc.text(`INV-${new Date().getFullYear()}-${Date.now().toString().slice(-3)}`, summaryBoxX + 5, currentY + 7);
  ...
  const summaryX = rightCol - 70;
  doc.setFont("helvetica", "bold");
  doc.setFontSize(9);
  doc.setTextColor(...cTextBlack);
  doc.text("TOTAL SUMMARY", summaryX, currentY + 2);

  doc.setFont("helvetica", "normal");
  doc.setFontSize(8.5);
  doc.setTextColor(...cTextGray);
  doc.text("Subtotal", summaryX, currentY + 12);
  doc.text(formatCurrency(data.total), rightCol, currentY + 12, { align: "right" });

  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  doc.setTextColor(...cNavyHeader);
  doc.text("Net Payable", summaryX, currentY + 20);
  doc.text(formatCurrency(data.total), rightCol, currentY + 20, { align: "right" });

  // Amount in words
  doc.setFont("helvetica", "normal");
  doc.setFontSize(7.5);
  doc.setTextColor(...cTextGray);
  doc.text(toWords(data.total).toUpperCase(), rightCol, currentY + 27, { align: "right" });
  ...
  let finalY = (doc.lastAutoTable && doc.lastAutoTable.finalY) || currentY + 40;

  // Small Summary Line below table
  doc.setFontSize(8);
  doc.setFont("helvetica", "bold");
  doc.setTextColor(...cTextBlack);
  doc.text("Grand Total", summaryX, finalY + 10);
  doc.text(formatCurrency(data.total), rightCol, finalY + 10, { align: "right" });

  finalY += 30;

  // ── 5. Thank You & Signatory ────────────────────
  doc.setFontSize(14);
  doc.setFont("helvetica", "normal");
  doc.setTextColor(...cTextBlack);
  doc.text("Thank you for your business!", centerX, finalY, { align: "center" });
  ...
  doc.setFontSize(8);
  doc.setTextColor(...cWhite);
  doc.text("www.kitsengineering.com", centerX, pageHeight - 5, { align: "center" });
